...
with DAG(
    ...
    t5 = BashOperator(task_id="removiendo_archivo", bash_command="rm -rf test_sensor.txt")

    # [START example_file_sensor]
    t6 = FileSensor(task_id="wait_for_file", filepath="test_sensor.txt")
    # [END example_file_sensor]

    # [START example_file_sensor_async]
    t7 = FileSensor(
        task_id="wait_for_file_async", filepath="test_sensor.txt", deferrable=True
    )
    # [END example_file_sensor_async]

    t8 = BashOperator(
        task_id="Creando_Archivo", bash_command="sleep 3; touch test_sensor.txt"
    )
